#!/usr/bin/env python3
"""
Permutation test on Proverbs 10–29 — does the true Hebrew ordering produce
more recurring catchword pairs than random shuffles, and does Hebrew (the
SOURCE language) lead the cross-linguistic comparison?

If Hebrew leads decisively → pipeline validated (positive control).
If Hebrew does not lead → pipeline has a blind spot; Thomas null softens.

Output:
  data/proverbs/permutation/main_results.json
"""
...
HEB_FILE = REPO_ROOT / "data" / "proverbs" / "proverbs_hebrew.json"
TRANS_DIR = REPO_ROOT / "data" / "proverbs" / "translations"
OUT = REPO_ROOT / "data" / "proverbs" / "permutation" / "main_results.json"
...
def load_translations(lang: str, variant_idx: int = 0
                       ) -> dict[int, list[dict]]:
    """Return {unit_id: tokens}."""
...
def precompute_matrix(translations, blocked, lang, ids):
    ...
    t0 = time.time()
    print(f"  Precomputing {len(ids)*(len(ids)-1)} cells…")
...
def main() -> None:
    ...
    ap.add_argument("--languages", default="hebrew,greek,syriac,aramaic,arabic")
    ...
    print(f"=== Proverbs permutation test ===")
    print(f"  N units (with Hebrew text): {len(ids_master)}")
    ...
    for lang in langs:
        ...
        print(f"  loaded {n_loaded}/{len(ids_master)} units")
    ...
    print()
    print("=" * 80)
    print(f"{'Lang':10s} {'True (≥2)':>10s} {'Null':>10s} {'std':>6s} "
          f"{'z':>6s} {'p':>10s}")
    print("-" * 80)
    for lang in langs:
        s = summary.get(lang)
