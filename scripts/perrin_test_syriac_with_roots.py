#!/usr/bin/env python3
"""
Re-run Thomas Syriac (surface forms) with the SEDRA lemma→root table ENABLED
in the detector. This catches pairs of distinct lemmas that share a
triliteral root as 'etymological' catchwords — closer to what scholars mean
by "shared root" than the consonantal-skeleton equality fallback.

Specifically tests outstanding-work item #3 in a lightweight form: extend
the phon/etym detector with a real lemma→root table rather than skeleton
heuristics alone.

Output: data/perrin_direct/thomas_syriac_v0_surface_with_roots.json
        data/perrin_direct/vanilla_thomas_syriac_v0_surface_with_roots.json
"""
...
from scripts.perrin_test_vanilla import classify_vanilla  # noqa: E402

OUT_DIR = REPO_ROOT / "data" / "perrin_direct"
LEMMA_TO_ROOT = REPO_ROOT / "data" / "processed" / "syriac_lemma_to_root.json"
...
def load_syriac_surface() -> dict[int, list[dict]]:
    ...
    for i in range(N_LOGIA):
        ...
        if vs and vs[0].get("success"):
            text = vs[0]["syriac_text"]
            out[i] = [{"form": t, "lemma": t, "parse": "MS-EMP"}
                       for t in tokenize(text, "syriac")]
    return out
...
def precompute_matrix(translations, blocked, ids, mode: str,
                       lemma_to_root: dict | None):
    """mode = 'default' (CatchwordDetector with SEDRA roots) or 'vanilla'."""
    if mode == "default":
            ...
            if mode == "default":
                ...
                for tok_a in ta:
                    ...
                    for tok_b in tb:
                        ...
                        res = classify_vanilla(la, lb, PHON_THRESHOLD)
                        if res:
                            seen[k] = res[0]
...
def run_test(toks, blocked, ids, mode: str, lemma_to_root):
    print(f"\n=== Syriac SURFACE Thomas, mode={mode}, SEDRA roots={'YES' if lemma_to_root else 'NO'} ===")
    matrix = precompute_matrix(toks, blocked, ids, mode, lemma_to_root)
    ...
    print(f"  TRUE adj: phon={n_phon}, etym={n_etym}, sem={n_sem}")
    results = {}
    ...
    for fname, lf in FILTERS.items():
        ...
        print(f"  [{fname:4s}] true={true_t}, null={nm:.1f}±{ns:.1f}, "
              f"z={z:.2f}, p={pv:.4f}")
    return results, {"phonological_total": n_phon, "etymological_total": n_etym,
...
def main():
    ...
    lemma_to_root = json.loads(LEMMA_TO_ROOT.read_text(encoding="utf-8"))
    print(f"  {len(lemma_to_root)} entries")

    print("Loading Syriac Thomas with SURFACE forms (no SEDRA-lemmatize)...")
    ...
    # Default detector + SEDRA roots
    results_default, diag_default = run_test(toks, blocked, ids,
                                                  "default", lemma_to_root)

    OUT_DIR.mkdir(parents=True, exist_ok=True)
    out1 = OUT_DIR / "thomas_syriac_v0_surface_with_roots.json"
    out1.write_text(json.dumps({
        "corpus": "thomas", "lang": "syriac", "variant": 0,
        "is_source": True, "tokenization": "surface_forms_no_sedra_lemma",
        "detector": "default_syriac_profile_with_SEDRA_roots",
        "n_units_used": len(ids), "n_blocked": len(blocked),
        "diagnostic": diag_default,
